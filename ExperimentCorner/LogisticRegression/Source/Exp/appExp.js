...
slider.onchange = function() {
  ...
  vizDecisionBoundary(
    (dataX0 = mIrisX_c0),
    (dataX1 = mIrisX_c1),
    (classifierFn = model.classify),
    (divName = "logisticRegressionViz"),
    window
  );
};
...
vizDecisionBoundary(
  (dataX0 = mIrisX_c0),
  (dataX1 = mIrisX_c1),
  (classifierFn = model.classify.bind(model)),
  (divName = "logisticRegressionViz")
);

function vizDecisionBoundary(
  dataX0,
  dataX1,
  classifierFn,
  divName = "",
  ...
  // creating grid Mesh
  const psudoPts0 = tf
    .linspace(gridRange0[0], gridRange0[1], gridRes)
    .expandDims(1);
  const psudoPts1 = tf
    .linspace(gridRange1[0], gridRange1[1], gridRes)
    .expandDims(1);
  const psudoPts = psudoPts0.concat(psudoPts1, (axis = 1));
  const meshGridPsudoPts = meshGrid(
    psudoPts1.flatten().arraySync(),
    psudoPts0.flatten().arraySync()
  );

  // classifying each point
  const meshGridPsudoPtsY = meshGridPsudoPts.map(function(cRow) {
    // console.log(cRow);
    const cRowClass = classifierFn(tf.tensor(cRow).transpose());

    // console.log("printing cRowClass");
    // cRowClass.print();
    if (cRowClass.shape[1] === 1) return cRowClass.flatten().arraySync();
  ...
  window.Plotly.newPlot(document.getElementById(divName), inputSpaceVizData, {
    title: "input Space"
  });
  ...
  const gridMeshLogit = meshGridPsudoPts.map(function(cRow) {
    // console.log(cRow);
    return model
      .logisticFn((threshold = 0.5), (convert2Class = false))(
        tf.tensor(cRow).transpose(),
        currWeight
      )
      .flatten()
      .arraySync();
  ...
  Plotly.newPlot("sigmoidCurveViz", logitVizData, {
    title: "Logit Function Viz"
  });
}
