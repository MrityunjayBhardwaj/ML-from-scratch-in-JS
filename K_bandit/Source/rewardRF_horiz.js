...
let rewardRFData = rewardRFArray.map((d,i)=>{ return {x: .3,y:i}})

  // Add X axis
  var rewardRFScaleX = d3.scaleBand()
    .range([ 0, width])
  ...
  // Y axis
  var rewardRFScaleY = d3.scaleLinear()
    .domain([0, 1])
    .range([ height, 0 ]);
  rewardRFSvg.append("g")
  ...
  //Bars
    ...
    .enter()
    .append("rect")
    .attr("y", (d,i)=>{return rewardRFScaleY(d.x)})
    .attr("x", function(d) { return rewardRFScaleX(d.y); })
    .attr("height", function(d) { return rewardRFScaleY(1-d.x); })
    .attr("width", rewardRFScaleX.bandwidth() )
    .attr("fill", "#69b3a2")
